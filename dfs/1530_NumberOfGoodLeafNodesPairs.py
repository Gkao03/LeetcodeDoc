...
# Space: O(nd). n is number f nodes in tree. d is distance.
...
class Solution:
    def dfs(self, curr_node, hashmap):
        if curr_node.left is None and curr_node.right is None:
            new_hashmap = deque([0] * (self.distance + 1))
            new_hashmap[1] = 1
            return new_hashmap

        left_hashmap = deque([0] * (self.distance + 1))
        if curr_node.left is not None:
            new_hashmap = copy.deepcopy(hashmap)

            # shift
            new_hashmap.rotate(1)
            new_hashmap[0] = 0

            # call left
            left_hashmap = self.dfs(curr_node.left, new_hashmap)

        right_hashmap = deque([0] * (self.distance + 1))
        ...
        for dist in range(1, self.distance):
            right_sum = sum(list(itertools.islice(right_hashmap, 0, self.distance - dist + 1)))
            self.answer += left_hashmap[dist] * right_sum

        merged_hashmap = deque([0] * (self.distance + 1))
    ...
    def countPairs(self, root: TreeNode, distance: int) -> int:
        ...
        self.answer = 0
        self.dfs(root, deque([0] * (distance + 1)))
